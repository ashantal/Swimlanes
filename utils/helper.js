...
module.exports = function (config_filename, logger) {
	...
	// get the channel id on network for listings
	...
	helper.getListingsPort = function () {
		return getListingsField('port');
	};

	// get the status of listings previous startup
	...
	helper.getKeepAliveMs = function () {
		var sec = getListingsField('keep_alive_secs');
		if (!sec) sec = 30;									//default to 30 seconds
		return (sec * 1000);
	};

	// safely retrieve listings fields
	function getListingsField(listings_field) {
		try {
			if (helper.config[listings_field]) {
				return helper.config[listings_field];
			}
			else {
				logger.warn('"' + listings_field + '" not found in config json: ' + helper.config_path);
				return null;
			}
		}
		catch (e) {
	...
	helper.makeUniqueId = function () {
		...
		return 'listings-' + helper.getNetworkName() + '-' + channel + '-' + first_peer;
	};

	// build the listings lib module options
	helper.makeListingsLibOptions = function () {
		const channel = helper.getChannelId();
	...
	// check if listings UI and listings chaincode work together
